        ...
        if (empty($id_user)) {
            ...
            if ($id_user == $_SESSION['user_id'] && $role != $_SESSION['role']) {
                ...
                if (!empty($password)) {
                    ...
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                    // PERBAIKAN: Menggunakan id_user di WHERE clause
                    $sql = "UPDATE users SET nama_lengkap=?, username=?, password=?, role=? WHERE id_user=?";
                    $stmt = $conn->prepare($sql);
...
if (isset($_GET['action']) && $_GET['action'] == 'hapus' && isset($_GET['id'])) {
    ...
    // Perbaikan pengambilan session ID
    $id_admin_sekarang = isset($_SESSION['user']['id_user']) ? $_SESSION['user']['id_user'] : (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 0);
    ...
    if ($id_hapus === $id_admin_sekarang) {
        ...
        // PERBAIKAN: Menggunakan id_user
        $stmt_hapus = $conn->prepare("DELETE FROM users WHERE id_user = ?");
...
// PERBAIKAN: SELECT id_user
$semua_pengguna = $conn->query("SELECT id_user, username, nama_lengkap, role FROM users ORDER BY role, username ASC");
?>
                            ...
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <!-- PERBAIKAN: Menggunakan id_user di parameter JS -->
                                        ...
                                        <?php 
                                        // Perbaikan logika cek ID admin saat ini
                                        $current_admin_id = isset($_SESSION['user']['id_user']) ? $_SESSION['user']['id_user'] : (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 0);
                                        
                                        if ((int)$pengguna['id_user'] !== (int)$current_admin_id): 
                                        ?>
                                            <a href="kelola_pengguna.php?action=hapus&id=<?= $pengguna['id_user'] ?>" 
